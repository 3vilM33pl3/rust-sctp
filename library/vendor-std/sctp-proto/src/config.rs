...
pub struct TransportConfig {
    pub(crate) random_source: fn(&mut [u8]),
    pub(crate) max_init_timeout_ms: Option<u64>,
...
impl TransportConfig {
    /// Cap INIT/COOKIE retransmission intervals without changing data RTOs.
    pub fn with_max_init_timeout_ms(mut self, milliseconds: u64) -> Self {
        self.max_init_timeout_ms = Some(milliseconds.max(1));
        self
    }
    /// Provide entropy for initial TSNs and handshake cookies.
    pub fn with_random_source(mut self, random: fn(&mut [u8])) -> Self {
...
impl EndpointConfig {
    /// Provide entropy for verification tags generated by this endpoint.
    pub fn with_random_source(mut self, random: fn(&mut [u8])) -> Self {
        self.aid_generator_factory = Arc::new(move || Box::new(crate::util::CallbackAssociationIdGenerator(random)));
        self
    }
...
pub struct ClientConfig {
    ...
    pub transport: Arc<TransportConfig>,
    /// Optional local SCTP source port to encode in the common header.
    pub(crate) local_sctp_port: Option<u16>,
    /// Optional remote SCTP destination port to encode in the common header.
    pub(crate) remote_sctp_port: Option<u16>,
...
impl ClientConfig {
    ...
    /// Override the SCTP source and destination ports carried inside the SCTP common header.
    pub fn with_sctp_ports(mut self, local_sctp_port: u16, remote_sctp_port: u16) -> Self {
        self.local_sctp_port = Some(local_sctp_port);
        self.remote_sctp_port = Some(remote_sctp_port);
        self
    }
}
