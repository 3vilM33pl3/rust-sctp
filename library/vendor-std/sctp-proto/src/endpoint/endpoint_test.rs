...
/// Local stand-in for the `assert_matches` crate, which must not reach the
/// standard library's dependency graph.
macro_rules! assert_matches {
    ($e:expr, $($pat:pat_param)|+ $(if $guard:expr)? $(,)?) => {
        match $e {
            $($pat)|+ $(if $guard)? => {}
            _ => panic!(
                "assertion failed: value does not match `{}`",
...
fn test_association_handle_packet_before_init() -> Result<()> {
    ...
    for (name, packet) in tests {
        ...
        let mut a = Association::new(None, config, 1400, 0, remote, None, 5000, 5000, Instant::now());
