...
impl PayloadProtocolIdentifier {
    pub const Dcep: Self = Self(50);
    pub const String: Self = Self(51);
    pub const Binary: Self = Self(53);
    pub const StringEmpty: Self = Self(56);
    pub const BinaryEmpty: Self = Self(57);
...
impl fmt::Display for PayloadProtocolIdentifier {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match *self {
            PayloadProtocolIdentifier::Dcep => f.write_str("WebRTC DCEP"),
            PayloadProtocolIdentifier::String => f.write_str("WebRTC String"),
            PayloadProtocolIdentifier::Binary => f.write_str("WebRTC Binary"),
            PayloadProtocolIdentifier::StringEmpty => f.write_str("WebRTC String (Empty)"),
            PayloadProtocolIdentifier::BinaryEmpty => f.write_str("WebRTC Binary (Empty)"),
            other => write!(f, "Payload Protocol Identifier {}", other.0),
        }
    }
}
...
impl From<u32> for PayloadProtocolIdentifier {
    fn from(v: u32) -> PayloadProtocolIdentifier {
        PayloadProtocolIdentifier(v)
    }
}

impl From<PayloadProtocolIdentifier> for u32 {
    fn from(v: PayloadProtocolIdentifier) -> Self {
        v.0
    }
}
...
impl Chunk for ChunkPayloadData {
    ...
    fn marshal_to(&self, writer: &mut BytesMut) -> Result<usize> {
        ...
        writer.put_u32(u32::from(self.payload_type));
