...
use crate::config::{ClientConfig, DEFAULT_SCTP_PORT, EndpointConfig, ServerConfig, TransportConfig};
...
impl Endpoint {
    ...
    pub fn connect_at(
        ...
        let source_port = config.local_sctp_port.unwrap_or(DEFAULT_SCTP_PORT);
        let destination_port = config.remote_sctp_port.unwrap_or(DEFAULT_SCTP_PORT);

        match (config.local_sctp_init, config.remote_sctp_init) {
            (Some(local_init), Some(remote_init)) => {
                self.connect_with_snap(
                    config.transport,
                    remote,
                    source_port,
                    destination_port,
