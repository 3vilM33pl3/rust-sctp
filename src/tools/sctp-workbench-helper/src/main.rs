...
use std::net::{SctpListener, SctpSendInfo, SctpStream, SocketAddr};
use std::str::FromStr;
...
fn parse_args() -> Result<Options, String> {
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            ...
            "--expect-failure" => {
                opts.expect_failure =
                    Some(args.next().ok_or("missing value for --expect-failure")?);
            }
            "--read-messages" => {
...
fn run_server(opts: &Options) -> io::Result<()> {
    let bind = opts.bind.ok_or_else(|| {
        io::Error::new(io::ErrorKind::InvalidInput, "server mode requires --bind")
    })?;
...
fn run_client(opts: &Options) -> io::Result<()> {
    let connect = opts.connect.ok_or_else(|| {
        io::Error::new(io::ErrorKind::InvalidInput, "client mode requires --connect")
    })?;
...
fn emit_notify(notification: &str) {
    println!("{{\"event\":\"notify\",\"message\":\"{}\"}}", escape_json(notification));
}
